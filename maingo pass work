...
type Config struct {
	...
	// PremiumTelegramUsername — agar set hai, password-gate page par
	// password box ke NEECHE premium plans dikhte hain (10/20/30 din).
	// Kisi plan par tap karne se seedha is Telegram username ko
	// pre-filled message ke saath DM khulta hai. Empty = section hide.
	PremiumTelegramUsername string // PREMIUM_TELEGRAM_USERNAME
}
...
func loadConfig() (*Config, error) {
	...
	cfg.PremiumTelegramUsername = strings.TrimPrefix(strings.TrimSpace(os.Getenv("PREMIUM_TELEGRAM_USERNAME")), "@")
	if cfg.PremiumTelegramUsername == "" {
		cfg.PremiumTelegramUsername = "raj_dev_01"
	}
...
func renderPasswordPrompt(w http.ResponseWriter, slug string, wrong bool, videoURL, premiumUsername string) {
	msg := ""
	if wrong {
		msg = `<p style="color:#f87171;margin:0 0 14px;">❌ Galat password, dobara try karo.</p>`
	}
...
<div style="margin-bottom:6px;">
  ...
  background:linear-gradient(90deg,#7aa2f7,#bb86fc,#7aa2f7);background-size:200%% auto;
  -webkit-background-clip:text;background-clip:text;color:transparent;
  animation:ppGlow 3s linear infinite,ppShift 4s ease-in-out infinite;">
    👋 Hi, this is Raj
  </div>
  <div style="font-size:12.5px;color:#9aa0b4;margin-top:2px;letter-spacing:.3px;">
    Welcome — watch below to unlock your movie
  </div>
</div>
<div style="position:relative;margin:14px 0 16px;border-radius:12px;overflow:hidden;
box-shadow:0 0 0 1px rgba(122,162,247,.25),0 8px 24px rgba(0,0,0,.5);">
...
style="position:absolute;bottom:10px;right:10px;background:rgba(15,17,23,.75);color:#fff;
...
<style>
@keyframes ppGlow{0%%{filter:drop-shadow(0 0 2px rgba(122,162,247,.6))}50%%{filter:drop-shadow(0 0 10px rgba(187,134,252,.9))}100%%{filter:drop-shadow(0 0 2px rgba(122,162,247,.6))}}
@keyframes ppShift{0%%{background-position:0%% center}50%%{background-position:100%% center}100%%{background-position:0%% center}}
</style>`,
			html.EscapeString(videoURL))
	}
	// Premium plans — shown below the password box. Tapping a plan opens a
	// Telegram DM to the admin with a pre-filled message (plan + which link
	// they want unlocked), so the sale happens as a normal Telegram chat.
	premiumBlock := ""
	if premiumUsername != "" {
		type plan struct{ Days, Price, Label string }
		plans := []plan{
			{"10", "15", "10 din"},
			{"20", "25", "20 din"},
			{"30", "30", "30 din"},
		...
		for _, p := range plans {
			text := fmt.Sprintf("Hi! Mujhe %s ka premium chahiye (₹%s) — link: %s", p.Label, p.Price, slug)
			href := fmt.Sprintf("https://t.me/%s?text=%s", url.QueryEscape(premiumUsername), url.QueryEscape(text))
			fmt.Fprintf(&btns, `
<a href="%s" target="_blank" style="display:block;text-decoration:none;background:#22242f;
border:1px solid #333;border-radius:10px;padding:10px 14px;margin-bottom:8px;
color:#e6e6e6;font-size:14px;display:flex;justify-content:space-between;align-items:center;">
<span>%s</span><span style="color:#7ee787;font-weight:700;">₹%s</span></a>`,
				href, html.EscapeString(p.Label), html.EscapeString(p.Price))
		}
		premiumBlock = fmt.Sprintf(`
<div style="margin-top:18px;padding-top:16px;border-top:1px solid #2a2c3a;text-align:left;">
<div style="font-size:13px;color:#9aa0b4;margin-bottom:10px;text-align:center;">
🔑 Password nahi pata? Premium lo, seedha Telegram pe baat karo 👇
</div>
%s
</div>`, btns.String())
	}
...
<body style="background:#0f1117;color:#e6e6e6;font-family:system-ui,sans-serif;
display:flex;align-items:center;justify-content:center;min-height:100vh;margin:0;padding:20px 0;">
<form method="GET" action="/watch/%s" style="background:#1b1c26;padding:32px;border-radius:14px;
width:min(360px,90%%);text-align:center;box-shadow:0 8px 30px rgba(0,0,0,.4);">
<div style="font-size:36px;margin-bottom:10px;">🔒</div>
<h2 style="margin:0 0 14px;font-size:18px;">Password Protected Link</h2>
%s
%s
<input type="password" name="pw" placeholder="Enter password" autofocus required
style="width:100%%;box-sizing:border-box;padding:12px;border-radius:8px;border:1px solid #333;
background:#0f1117;color:#fff;font-size:15px;margin-bottom:14px;">
<button type="submit" style="width:100%%;padding:12px;border:none;border-radius:8px;
background:#7aa2f7;color:#0f1117;font-weight:700;font-size:15px;cursor:pointer;">Unlock</button>
%s
</form></body></html>`, slug, videoBlock, msg, premiumBlock)
}
...
func (a *App) handleWatch(w http.ResponseWriter, r *http.Request, slug string) {
	...
	// freshly submitted ?pw= from the prompt form below.
	if passwordHash != nil && *passwordHash != "" {
		...
		if !unlocked {
			if pw := r.URL.Query().Get("pw"); pw != "" {
				if sha256Hex(pw) == *passwordHash {
					...
					renderPasswordPrompt(w, slug, true, a.cfg.PasswordPromptVideoURL, a.cfg.PremiumTelegramUsername)
					return
				}
			}
		}
		if !unlocked {
			renderPasswordPrompt(w, slug, false, a.cfg.PasswordPromptVideoURL, a.cfg.PremiumTelegramUsername)
			return
		}
	}
