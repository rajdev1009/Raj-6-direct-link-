...
func (db *DB) getOrCreateApproval(ctx context.Context, deviceID, slug string) (rec *ApprovalRecord, isNew bool, err error) {
	rec = &ApprovalRecord{}
	err = db.pool.QueryRow(ctx,
		`SELECT access_id, device_id, slug, visitor_name, approved, created_at, approved_at
		 FROM approvals WHERE device_id=$1`, deviceID).
		Scan(&rec.AccessID, &rec.DeviceID, &rec.Slug, &rec.VisitorName, &rec.Approved, &rec.CreatedAt, &rec.ApprovedAt)
	if err == nil {
...
func (db *DB) listApprovals(ctx context.Context, limit int) ([]*ApprovalRecord, error) {
	...
	for rows.Next() {
		rec := &ApprovalRecord{}
		if err := rows.Scan(&rec.AccessID, &rec.DeviceID, &rec.Slug, &rec.VisitorName, &rec.Approved, &rec.CreatedAt, &rec.ApprovedAt); err != nil {
			return nil, err
		}
		out = append(out, rec)
	}
