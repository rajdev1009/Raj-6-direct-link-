...
func (a *App) handleStream(w http.ResponseWriter, r *http.Request, slug string, download bool) {
	...
	if cached != nil {
		...
		a.cache.setFile(ctx, slug, &cachedFile{
			...
			ExpiresAt: expiresAt, PasswordHash: passwordHash, PasswordPlain: rec.PasswordPlain,
		})
	}
...
func renderPasswordPrompt(w http.ResponseWriter, slug string, wrong bool, videoURL string, promptImages []string, contactTelegram, contactInstagram string) {
	msg := ""
	if wrong {
...
setTimeout(function(){el.src=next;el.style.opacity=1;},400);
	...
	// to ask for the password: Telegram username (text link) + Instagram
	// icon (links to the profile).
	contactBlock := ""
	if contactTelegram != "" || contactInstagram != "" {
		var links strings.Builder
		if contactTelegram != "" {
			fmt.Fprintf(&links, `
<a href="https://t.me/%s" target="_blank" style="display:inline-flex;align-items:center;gap:6px;
text-decoration:none;color:#7aa2f7;font-size:13.5px;font-weight:600;">
✈️ @%s</a>`, url.QueryEscape(contactTelegram), html.EscapeString(contactTelegram))
		}
		if contactInstagram != "" {
			fmt.Fprintf(&links, `
<a href="https://instagram.com/%s" target="_blank" style="display:inline-flex;align-items:center;gap:6px;
text-decoration:none;color:#e4405f;font-size:13.5px;font-weight:600;">
📸 @%s</a>`, url.QueryEscape(contactInstagram), html.EscapeString(contactInstagram))
		}
		contactBlock = fmt.Sprintf(`
<div style="margin-top:16px;padding-top:14px;border-top:1px solid #2a2c3a;
...
%s
%s
</form></body></html>`, slug, videoBlock, msg, imageBlock, contactBlock)
}
...
func (a *App) handleWatch(w http.ResponseWriter, r *http.Request, slug string) {
	...
	if passwordHash != nil && *passwordHash != "" {
		unlocked := hasValidPasswordCookie(r, *passwordHash)
		if !unlocked {
			if pw := r.URL.Query().Get("pw"); pw != "" {
				if sha256Hex(pw) == *passwordHash {
					...
					renderPasswordPrompt(w, slug, true, a.cfg.PasswordPromptVideoURL, a.cfg.PasswordPromptImages, a.cfg.ContactTelegramUsername, a.cfg.ContactInstagramUsername)
					return
				}
			}
		}
		if !unlocked {
			renderPasswordPrompt(w, slug, false, a.cfg.PasswordPromptVideoURL, a.cfg.PasswordPromptImages, a.cfg.ContactTelegramUsername, a.cfg.ContactInstagramUsername)
			return
		}
	}
...
// Password cookie is SITE-WIDE (not per-file). If the same password is used
// across multiple files (e.g. all your science + math videos share one
// premium password), unlocking once via any file auto-unlocks every other
// file that uses that same password — no repeated password prompts.
const passwordCookieName = "raj_pw_unlock"

func hasValidPasswordCookie(r *http.Request, hash string) bool {
	ck, err := r.Cookie(passwordCookieName)
	return err == nil && ck.Value == hash
}

func setPasswordCookie(w http.ResponseWriter, hash string) {
