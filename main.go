...
func (a *App) onCallback(ctx context.Context, cb *tgbotapi.CallbackQuery) {
	...
	if strings.HasPrefix(cb.Data, "approve_") || strings.HasPrefix(cb.Data, "block_") {
		if cb.From.ID != a.cfg.AdminID {
			a.pool.primary().Request(tgbotapi.NewCallback(cb.ID, "❌ Admin only.")) //nolint:errcheck
			return
		}
		isApprove := strings.HasPrefix(cb.Data, "approve_")
		idStr := strings.TrimPrefix(cb.Data, "approve_")
		if !isApprove {
			idStr = strings.TrimPrefix(cb.Data, "block_")
		}
		accessID, convErr := strconv.Atoi(idStr)
		if convErr != nil {
			a.pool.primary().Request(tgbotapi.NewCallback(cb.ID, "❌ Bad ID.")) //nolint:errcheck
			return
		}
		var found bool
		var dbErr error
		...
		if isApprove {
			found, dbErr = a.db.approveByID(ctx, accessID)
			resultLine = fmt.Sprintf("✅ Approved — %05d", accessID)
		} else {
			found, dbErr = a.db.blockByID(ctx, accessID, true)
			resultLine = fmt.Sprintf("🚫 Blocked — %05d", accessID)
		}
		if dbErr != nil {
			a.pool.primary().Request(tgbotapi.NewCallback(cb.ID, "❌ Failed, try again.")) //nolint:errcheck
			return
		}
		if !found {
			a.pool.primary().Request(tgbotapi.NewCallback(cb.ID, "❌ ID not found.")) //nolint:errcheck
			return
		}
		a.pool.primary().Request(tgbotapi.NewCallback(cb.ID, resultLine)) //nolint:errcheck
		// Edit the original message so the buttons disappear and the admin
		// can see at a glance what was decided, without cluttering the chat.
		edit := tgbotapi.NewEditMessageText(cb.Message.Chat.ID, cb.Message.MessageID,
			cb.Message.Text+"\n\n"+resultLine)
		a.pool.primary().Send(edit) //nolint:errcheck
		return
...
func (a *App) notifyNewAccessID(isNew bool, accessID int, fileName, visitorName string) {
	...
	kb := tgbotapi.NewInlineKeyboardMarkup(
		tgbotapi.NewInlineKeyboardRow(
			tgbotapi.NewInlineKeyboardButtonData("✅ Approve", fmt.Sprintf("approve_%d", accessID)),
			tgbotapi.NewInlineKeyboardButtonData("🚫 Block", fmt.Sprintf("block_%d", accessID)),
		),
	)
	a.pool.sendKB(a.cfg.AdminID, text, kb)
}

// renderHomepage is the public "/" page — Astratoonix branding plus a
// compact browse widget tucked into the bottom-right corner. The widget
// stays collapsed (just a small round button) until tapped, so it never
// takes up page real estate; opened, it reuses the same /search endpoint
// as the watch page so results (and locked/unlocked state) stay consistent
// everywhere.
func (a *App) renderHomepage(w http.ResponseWriter, r *http.Request) {
	devContact := a.cfg.ContactTelegramUsername
	if devContact == "" {
		devContact = "raj_dev_01"
	}
	subjects := []string{
	...
	fmt.Fprintf(w, `<!DOCTYPE html><html><head><meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1">
<title>Astratoonix — Learn Science, Math & Coding</title>
<link href="https://fonts.googleapis.com/css2?family=JetBrains+Mono:wght@500;700;800&family=Inter:wght@400;500;600;700&display=swap" rel="stylesheet">
<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.1/css/all.min.css">
<style>
* { margin:0; padding:0; box-sizing:border-box; }
html,body { height:100%%; }
body {
  background:#05060b; color:#e7ecf7; font-family:'Inter',system-ui,sans-serif;
...
.stile { font-family:'JetBrains Mono',monospace; font-weight:800; font-size:26px; letter-spacing:2px; color:#38d6ff; white-space:nowrap; }
.hero {
...
.brand { font-family:'JetBrains Mono',monospace; font-weight:800; font-size:clamp(30px,8vw,52px); letter-spacing:2px; color:#38d6ff; }
.by { font-size:12px; letter-spacing:4px; color:#6b7bab; margin-top:6px; }
.by b { color:#9d7cf7; }
.tagline { max-width:420px; margin:20px 0 22px; font-size:14px; color:#9db3d9; line-height:1.6; }
.badges { display:flex; gap:6px; flex-wrap:wrap; justify-content:center; margin-bottom:26px; }
.badge {
  font-size:10.5px; font-weight:700; letter-spacing:.4px; padding:5px 9px; border-radius:20px;
  background:#0f1a2a; border:1px solid #1d3350; color:#6fe3ff; display:inline-flex; align-items:center; gap:5px;
}
.badge.free { color:#7dffb0; border-color:#1f4632; background:#0d1c15; }
.badge.https { color:#ffd166; border-color:#4a3a12; background:#1c1608; }
.stack-line { display:flex; flex-wrap:wrap; gap:6px; justify-content:center; }
.stack-chip {
  font-size:10.5px; font-weight:700; color:#8ea3cf; background:#0d1322;
  border:1px solid #1a2436; border-radius:6px; padding:4px 8px; font-family:'JetBrains Mono',monospace;
}
.collab {
  position:relative; z-index:1; width:min(400px,90%%); margin:30px auto 40px;
  background:linear-gradient(135deg,#1c1608,#0b0f1a); border:1px solid #4a3a12;
  border-radius:14px; padding:16px 18px; text-align:center;
}
.collab .h { font-size:13px; font-weight:800; color:#ffd166; margin-bottom:5px; }
...
.collab a.cta {
  display:inline-flex; align-items:center; gap:6px; text-decoration:none;
  background:#ffd166; color:#1c1608; font-weight:800; font-size:12.5px; padding:8px 16px; border-radius:20px;
}

/* ---------- Corner browse widget — collapsed by default, tucked bottom-right ---------- */
#browseFab {
  position:fixed; right:18px; bottom:18px; z-index:50; width:52px; height:52px; border-radius:50%%;
  background:linear-gradient(135deg,#00f0ff,#ff2df0); border:none; color:#05060b; font-size:20px;
  cursor:pointer; box-shadow:0 6px 20px rgba(0,240,255,.35);
}
#browsePanel {
  position:fixed; right:18px; bottom:80px; z-index:50; width:min(280px,80vw); max-height:70vh;
  background:#0b0f1a; border:1px solid #182034; border-radius:14px; padding:14px;
  box-shadow:0 20px 60px rgba(0,0,0,.55); display:none; overflow-y:auto;
}
#browsePanel.open { display:block; }
#browsePanel .ttl { font-size:11px; letter-spacing:.4px; color:#6b7bab; margin-bottom:8px; }
#browseSearch {
  width:100%%; box-sizing:border-box; padding:9px 11px; border-radius:8px; border:1px solid #1c2537;
  background:#060810; color:#fff; font-size:13px; margin-bottom:10px; font-family:'Inter',sans-serif;
}
.subj-row { display:flex; flex-wrap:wrap; gap:6px; margin-bottom:10px; }
.subj-btn {
  font-size:11px; font-weight:600; padding:6px 9px; border-radius:8px; border:1px solid #1a2436;
  background:#0d1322; color:#9db3d9; cursor:pointer; display:flex; align-items:center; gap:5px;
}
.subj-btn:hover, .subj-btn.active { background:#38d6ff; color:#05060b; }
#browseResults a {
  display:block; padding:8px 6px; border-bottom:1px solid #131a2a; text-decoration:none;
  color:#e7ecf7; font-size:12.5px; line-height:1.4;
}
#browseResults .empty { color:#6b7bab; font-size:12px; padding:8px 6px; }
</style></head>
<body>
...
<div class="hero">
  <div class="brand">ASTRATOONIX</div>
  <div class="by">BUILT BY <b>RAJ</b></div>
  <div class="tagline">Science, Math aur coding (Python, C++, Linux, JS, CSS) — sab kuchh ek jagah. Advanced, secure aur 100%% free.</div>
  <div class="badges">
    <span class="badge">🔒 PRIVATE</span>
    <span class="badge https">🔐 HTTPS</span>
    <span class="badge free">💯 FREE</span>
...
<button id="browseFab" title="Browse videos"><i class="fas fa-book-open"></i></button>
<div id="browsePanel">
  <div class="ttl">BROWSE BY SUBJECT</div>
  <div class="subj-row">
    <button class="subj-btn" data-q="science"><i class="fas fa-flask"></i> Science</button>
    <button class="subj-btn" data-q="math"><i class="fas fa-square-root-variable"></i> Math</button>
    <button class="subj-btn" data-q="english"><i class="fas fa-book"></i> English</button>
    <button class="subj-btn" data-q="python"><i class="fab fa-python"></i> Coding</button>
    <button class="subj-btn" data-q="history"><i class="fas fa-landmark"></i> History</button>
  </div>
  <input type="text" id="browseSearch" placeholder="Search videos...">
  <div id="browseResults"><div class="empty">Type a subject or use the buttons above.</div></div>
</div>

<script>
var fab = document.getElementById('browseFab');
...
function runSearch(q){
  fetch('/search?q=' + encodeURIComponent(q)).then(function(r){ return r.json(); }).then(function(data){
    ...
    if (!items.length) { results.innerHTML = '<div class="empty">Koi video nahi mila.</div>'; return; }
    results.innerHTML = items.map(function(it){
      return '<a href="/watch/' + encodeURIComponent(it.id) + '">' + (it.locked ? '🔒 ' : '') + esc(it.name) + '</a>';
    }).join('');
  }).catch(function(){ results.innerHTML = '<div class="empty">Search failed.</div>'; });
}
var timer = null;
input.addEventListener('input', function(){
  clearTimeout(timer);
  var q = input.value.trim();
  if (q.length < 2) { results.innerHTML = '<div class="empty">Type a subject or use the buttons above.</div>'; return; }
...
</body></html>`, subjectTiles.String(), url.QueryEscape(devContact), html.EscapeString(devContact))
}
...
func (a *App) handleWatch(w http.ResponseWriter, r *http.Request, slug string) {
	...
	// Password gate — checks cookie first (already unlocked earlier), then a
	// freshly POSTed pw/name from the form below. This MUST be POST, not
	// GET: a GET submission puts the password straight into the URL, where
	// it leaks into browser history, server access logs, and the Referer
	// header sent to any third-party asset (fonts, icons) the page loads.
	// r.FormValue reads POST body first, so nothing sensitive ever touches
		...
		if !unlocked {
			if pw := r.FormValue("pw"); pw != "" {
				if sha256Hex(pw) == *passwordHash {
					...
					approvalRec, _, _ := a.db.getOrCreateApproval(ctx, deviceID, slug)
					hadNoName := approvalRec == nil || approvalRec.VisitorName == ""
					accessID := 0
					if approvalRec != nil {
						accessID = approvalRec.AccessID
					}
					if visitorName != "" {
						if err := a.db.setApprovalName(ctx, deviceID, visitorName); err != nil {
							a.logger.Warn("setApprovalName failed", zap.Error(err))
						}
						// Notify the moment we actually have a name to show —
						// not just on a brand-new device. This fires exactly
						// once per device (hadNoName flips to false after this),
						// whether they got the password right on the first try
						// or the fifth.
						if hadNoName {
							a.notifyNewAccessID(true, accessID, fileName, visitorName)
						}
					}
					// Redirect-after-POST: the cookie is set, so a plain GET to
					// the same URL now serves the video directly. This avoids
					// the browser's "resubmit form?" warning on refresh, and
					// keeps the password itself out of any URL entirely.
					if r.Method == http.MethodPost {
						http.Redirect(w, r, "/watch/"+slug, http.StatusSeeOther)
						return
					}
				} else {
