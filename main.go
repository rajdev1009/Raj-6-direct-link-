...
func (a *App) onFile(ctx context.Context, msg *tgbotapi.Message) {
	...
	text := fmt.Sprintf(
		"✅ *File Stored\\!*\n\n📄 `%s`\n📦 `%s`\n\n▶️ [Stream](%s)\n⬇️ [Download](%s)\n📺 [Watch Online](%s)\n\n🔒 Permanent link\\!",
		mdEscape(fi.fileName), formatSize(fi.fileSize), streamLink, dlLink, watchLink,
	)
...
func renderPasswordPrompt(w http.ResponseWriter, slug string, wrong bool) {
	msg := ""
	if wrong {
...
width:min(320px,90%%);text-align:center;box-shadow:0 8px 30px rgba(0,0,0,.4);">
...
func (a *App) handleWatch(w http.ResponseWriter, r *http.Request, slug string) {
		...
		if !unlocked {
			if pw := r.URL.Query().Get("pw"); pw != "" {
				if sha256Hex(pw) == *passwordHash {
					...
					renderPasswordPrompt(w, slug, true)
					return
				}
			}
		}
		if !unlocked {
			renderPasswordPrompt(w, slug, false)
			return
		}
	}
	...
	data := WatchData{
		...
		MimeType:    mimeType,
		StreamURL:   streamURL,
		PlayItURL:   "playit://play?url=" + url.QueryEscape(streamURL),
...
func main() {
	...
	app.pool.sendMD(cfg.AdminID, fmt.Sprintf(
