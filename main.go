...
type FileRecord struct {
	...
	ViewCount    int64      // total /watch page visits
}
...
func (db *DB) getFileByID(ctx context.Context, id string) (*FileRecord, error) {
	...
	err := db.pool.QueryRow(ctx, `
		SELECT id,message_id,channel_id,file_name,file_size,mime_type,hash,uploader_id,uploader_name,created_at,expires_at,view_count
		FROM files WHERE id=$1`, id).Scan(
		&f.ID, &f.MessageID, &f.ChannelID, &f.FileName,
		&f.FileSize, &f.MimeType, &f.Hash,
		&f.UploaderID, &f.UploaderName, &f.CreatedAt,
		&f.ExpiresAt, &f.ViewCount,
	)
	return f, err
}
...
func (a *App) handleWatch(w http.ResponseWriter, r *http.Request, slug string) {
	...
	// Track this visit — persisted in DB, and notify the admin on Telegram at milestones
	views, err := a.db.incrementViews(ctx, slug)
	if err != nil {
		a.logger.Warn("incrementViews failed", zap.Error(err))
	} else {
		for _, m := range viewMilestones {
			if views == m {
				a.pool.sendMD(a.cfg.AdminID, fmt.Sprintf(
					"👀 *%s*\n\nhit *%s* views on the watch page\\!",
