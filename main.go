...
func (db *DB) getFileByID(ctx context.Context, id string) (*FileRecord, error) {
	...
	err := db.pool.QueryRow(ctx, `
		SELECT id,message_id,channel_id,file_name,file_size,mime_type,hash,uploader_id,uploader_name,created_at,expires_at,view_count,password_hash
		FROM files WHERE id=$1`, id).Scan(
		&f.ID, &f.MessageID, &f.ChannelID, &f.FileName,
		&f.FileSize, &f.MimeType, &f.Hash,
		&f.UploaderID, &f.UploaderName, &f.CreatedAt,
		&f.ExpiresAt, &f.ViewCount, &f.PasswordHash,
	)
	return f, err
}

// setPassword sets (hash != nil) or clears (hash == nil) the password for a link.
func (db *DB) setPassword(ctx context.Context, id string, hash *string) error {
	tag, err := db.pool.Exec(ctx, `UPDATE files SET password_hash=$1 WHERE id=$2`, hash, id)
	if err != nil {
		return err
	}
...
func (db *DB) topFilesByViews(ctx context.Context, limit int) ([]*FileRecord, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&f.ID, &f.MessageID, &f.ChannelID, &f.FileName,
			...
			&f.CreatedAt, &f.ExpiresAt, &f.ViewCount, &f.PasswordHash); err != nil {
			return nil, err
		}
		out = append(out, f)
	}
...
func (a *App) onCommand(ctx context.Context, msg *tgbotapi.Message) {
	...
	case "setpass":
		...
		if strings.EqualFold(parts[1], "off") {
			if err := a.db.setPassword(ctx, slug, nil); err != nil {
		...
		if err := a.db.setPassword(ctx, slug, &hash); err != nil {
...
func (a *App) handleStream(w http.ResponseWriter, r *http.Request, slug string, download bool) {
	...
	if passwordHash != nil && *passwordHash != "" && !hasValidPasswordCookie(r, slug, *passwordHash) {
...
func renderPasswordPrompt(w http.ResponseWriter, slug string, wrong bool, videoURL string) {
	msg := ""
	if wrong {
...
</form></body></html>`, slug, videoBlock, msg)
}
		...
		if !unlocked {
			if pw := r.URL.Query().Get("pw"); pw != "" {
				if sha256Hex(pw) == *passwordHash {
					...
					renderPasswordPrompt(w, slug, true, a.cfg.PasswordPromptVideoURL)
					return
				}
			}
		}
		if !unlocked {
			renderPasswordPrompt(w, slug, false, a.cfg.PasswordPromptVideoURL)
			return
		}
	}
...
th{color:#8b949e;font-weight:600;} .locked{color:#f0b429;}
</style></head><body>
...
<table><thead><tr><th>File</th><th>Size</th><th>Views</th><th>Live now</th><th>🔒</th><th>Uploaded</th></tr></thead><tbody>`,
		files, users, a.pool.count(), formatCount(totalViews), liveNow)

	for _, f := range top {
		lock := ""
		if f.PasswordHash != nil && *f.PasswordHash != "" {
			lock = `<span class="locked">🔒</span>`
		}
