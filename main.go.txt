...
func (db *DB) getOrCreateApproval(ctx context.Context, deviceID, slug string) (rec *ApprovalRecord, isNew bool, err error) {
	rec = &ApprovalRecord{}
	err = db.pool.QueryRow(ctx,
		`SELECT access_id, device_id, slug, approved, created_at, approved_at
		 FROM approvals WHERE device_id=$1`, deviceID).
		Scan(&rec.AccessID, &rec.DeviceID, &rec.Slug, &rec.Approved, &rec.CreatedAt, &rec.ApprovedAt)
	if err == nil {
...
func renderPasswordPrompt(w http.ResponseWriter, slug string, wrong bool, accessID int, videoURL string, promptImages []string, contactTelegram, contactInstagram string) {
	msg := ""
	if wrong {
...
text-decoration:none;color:#38d6ff;font-size:13.5px;font-weight:600;">
...
input[type=password]:focus { outline:none; border-color:#38d6ff; }
button.unlock {
...
</body></html>`, subjectTiles.String(), videoBlock, msg, accessIDBlock, slug, imageBlock, contactBlock, url.QueryEscape(devContact), html.EscapeString(devContact))
}
...
// without the visitor having to do anything.
func renderPendingApproval(w http.ResponseWriter, slug string, accessID int) {
	w.Header().Set("Content-Type", "text/html; charset=utf-8")
	fmt.Fprintf(w, `<!DOCTYPE html><html><head><meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1">
...
.idbox {
  padding:14px; border-radius:10px; background:#0d1322; border:1px solid #1a2436;
}
...
<div class="card">
  <div class="spin"></div>
  <h2>Password sahi hai ✅</h2>
  <p>Admin ne abhi tak approve nahi kiya. Yeh Access ID admin ko bhejo — approve hote hi yeh page apne aap khul jaayega.</p>
  <div class="idbox">
...
func (a *App) notifyNewAccessID(isNew bool, accessID int, fileName string) {
	if !isNew || accessID == 0 {
		return
	}
...
func (a *App) handleWatch(w http.ResponseWriter, r *http.Request, slug string) {
	...
	if passwordHash != nil && *passwordHash != "" {
		...
		if !unlocked {
			if pw := r.URL.Query().Get("pw"); pw != "" {
				if sha256Hex(pw) == *passwordHash {
					...
					a.notifyNewAccessID(isNewDevice, accessID, fileName)
					renderPasswordPrompt(w, slug, true, accessID, a.cfg.PasswordPromptVideoURL, a.cfg.PasswordPromptImages, a.cfg.ContactTelegramUsername, a.cfg.ContactInstagramUsername)
					return
				}
			}
		}
		if !unlocked {
			...
			a.notifyNewAccessID(isNewDevice, accessID, fileName)
			renderPasswordPrompt(w, slug, false, accessID, a.cfg.PasswordPromptVideoURL, a.cfg.PasswordPromptImages, a.cfg.ContactTelegramUsername, a.cfg.ContactInstagramUsername)
			return
		}

		// Password correct (or already unlocked earlier) — but that's only
		// half the gate. The admin must separately run /approve <accessID>
		// for this specific device before it can actually stream.
		...
		if approval == nil || !approval.Approved {
			...
			a.notifyNewAccessID(isNewDevice, accessID, fileName)
			renderPendingApproval(w, slug, accessID)
			return
		}
	}
